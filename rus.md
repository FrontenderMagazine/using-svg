...
же нажать кнопку “SVG Code...”, которая откроет окно TextEdit (по
...
наличие блоков SVG-кода в нем и невозможность кэширования.
...
генератора и прочий мусор. У SVG, в общем, и так небольшой размер, но
...
два составляющих элемента, `<ellipse>` и `<path>`. Можно просто открыть код 
...
Даже лучше, в SVG можно использовать фильтры, например размытие:
...
Добавление SVG непосредственно в документ по-разному [поддерживается браузерами][18], 
однако все сводится к отсутствию поддержки IE младше 8 и браузерам на Android
...
`<object>`. Вот что нужно добавить в SVG-файл перед открывающим тегом `<svg>`:
...
код заменяет `<img>` или `background-image`, система ругаться не будет, но и 
...
[Mobilefish.com][42] для этого есть [онлайн-конвертер][19]. Просто скопируйте
...
Кстати, если если добавить `<style>` в SVG до перекодирования в base64, [он будет

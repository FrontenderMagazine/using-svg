...
SVG - это формат векторной графики. Буквально его название значит
**Масштабируемая Векторная Графика** (Scalable Vector Graphics). Попросту
говоря, это то с чем вы работаете в Adobe Illustrator. SVG довольно просто
использовать в вебе, но сперва нужно много с чем разобраться.

## Зачем вообще нужен SVG?

* Небольшие размеры файлов, хорошее сжатие 
* Масштабирование до любого размера без потери качества (разве что при совсем 
маленьких размерах)
* Хорошо выглядит на мониторах ретина
* Возможность управлять внешним видом благодаря интерактивности и фильтрам
...
Обратите внимание что изображение кадрируется чётко по граням монтажной области. 
Холст в SVG играет не меньшую роль чем в PNG или JPG. 

Можете сохранить файл с расширением SVG прямо в Adobe Illustrator. 

![Сохранение][Сохраняем файл с расширением SVG]

При сохранении появится еще одно диалоговое окно с настройками. Честно говоря, я 
...
поддерживают вставку через `img`, все отображалось корректно, выход есть. Я 
...
Также можно обратиться к [SVGeezy][8]. Дальше в статье мы еще вернемся к 
рассмотрению других "костылей". 

## Вставка SVG через `background-image`

SVG так же можно добавить через CSS-свойство `background-image`, этот способ так 
же прост как и вставка с помощью `img`. 
...
Среди них вероятность получения раздутого до больших размеров документа, большой 
кусок мусора прямо посреди документа, который вы пытаетесь упорядочить, 
невозможность кеширования.
...
Наверняка для вас не станет сюрпризом то, что SVG, полученные в Adobe 
...
видео][12] Кайл Фостер заходит еще дальше и удаляет даже переносы строки по ходу 
оптимизации. 

Если вы истинный хардкоровец, вот вам [инструмент Node JS][13], с помощью 
...
иное как XML (проименованные теги в квадратных скобках и всякой всячиной внутри). 
...
Даже круче того, для SVG можно использовать всякие навороченные фильтры. К 
примеру, размытие. Прицепите к своему `<svg>` фильтр:
...
И потом его можно применить через CSS когда нужно:

    .ground:hover {
      filter: url(#pictureFilter);
    }

Вот [пример][14] что может получиться:
...
И да, если если встроить `<style>` в SVG до перекодирования в base64, [он будет 
